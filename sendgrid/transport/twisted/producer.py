...
class MultiPartProducer(object):
    ...
    def __init__(self, files=None, data=None, callback=None, deferred=None):
        """ Initialize.
        ...
        _data = data or {}
        self._data = {}
        for (key, field) in _data.items():
            if not isinstance(field, (str, unicode)):
                for (subkey, subfield) in enumerate(field):
                    self._data["%s[%s]" % (key, subkey)] = subfield
            else:
    ...
    def _boundary(self):
        ...
        try:
            ...
            boundary = hashlib.sha1(str(bits)).hexdigest()
        return boundary
    ...
    def _file_size(self, field):
        ...
        """
        try:
            handle = open(self._files[field], "rb")
